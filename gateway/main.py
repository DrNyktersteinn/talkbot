...
def _extract_ollama_text(j) -> str:
    # Accepts both /api/chat and /api/generate shapes
    try:
...
>>>>>>> 3cbc8fe (Initial commit: My TalkBot deployment)
...
=======
    ...
    try:
        ...
        # Retry once with fallback vision model if empty
...
async def vision_diag(
    ...
    # Build the exact same prompt as /vision
    text_prompt = (prompt or "").strip()
    nav_hint = {
    ...
    diag = {"status": None, "raw": None, "extracted": None, "model": None, "fallback_used": False, "error": None}
    try:
        async with httpx.AsyncClient(timeout=120.0) as c:
            payload = {"model": VISION_MODEL, "messages": [{"role":"user","content": q, "images":[img_b64]}], "stream": False}
            r = await c.post(f"{OLLAMA_URL}/api/chat", json=payload)
            ...
            try:
                ...
                j = None
            diag["raw"] = j
            diag["model"] = VISION_MODEL
            text = _extract_ollama_text(j) if j else ""
            if not (text or "").strip():
                fb = os.getenv("FALLBACK_VISION_MODEL", "").strip()
                if fb:
                    diag["fallback_used"] = True
                    payload["model"] = fb
                    r2 = await c.post(f"{OLLAMA_URL}/api/chat", json=payload)
                    ...
                    diag["raw"] = j2
                    diag["model"] = fb
                    text = _extract_ollama_text(j2) if j2 else ""
            diag["extracted"] = text or ""
    except Exception as e:
        diag["error"] = str(e)

    return {"diag": diag, "extracted_content": diag.get("extracted") or ""}
